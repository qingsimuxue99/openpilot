...
class Car:
  ...
  def state_update(self) -> tuple[car.CarState, custom.CarStateSP, structs.RadarDataT | None]:
    ...
    if self.experimental_mode and self.sm.valid['longitudinalPlanSP']:
      v_target_kph = self.sm['longitudinalPlanSP'].vTarget * CV.MS_TO_KPH
      if 0.0 < v_target_kph < V_CRUISE_MAX:
        CS.vCruiseCluster = float(v_target_kph)
      else:

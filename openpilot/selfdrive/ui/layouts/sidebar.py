...
NETWORK_TYPES = {
  ...
  NetworkType.wifi: tr_noop("无线"),
  NetworkType.ethernet: tr_noop("有线"),
...
class Sidebar(Widget, SidebarSP):
  def __init__(self):
    ...
    self._temp_status = MetricData(tr_noop("温度"), tr_noop("--°C"), Colors.GOOD)
    self._panda_status = MetricData(tr_noop("车辆"), tr_noop("在线"), Colors.GOOD)
    self._connect_status = MetricData(tr_noop("连接"), tr_noop("离线"), Colors.WARNING)
  ...
  def _update_network_status(self, device_state):
    self._net_type = NETWORK_TYPES.get(device_state.networkType.raw, tr_noop("未知"))
  ...
  def _update_temperature_status(self, device_state):
    ...
    if thermal_status == ThermalStatus.ok:
      self._temp_status.update(tr_noop("温度"), f"{int(cpu_temp)}°C", Colors.GOOD)
    else:
      self._temp_status.update(tr_noop("温度"), f"{int(cpu_temp)}°C", Colors.DANGER)

  def _update_connection_status(self, device_state):
    # 用本地网络状态判定：WiFi/以太网一连上就变「在线」，不再等 comma 云端心跳
    if device_state.networkType.raw == NetworkType.none:
      self._connect_status.update(tr_noop("连接"), tr_noop("离线"), Colors.WARNING)
    else:
      self._connect_status.update(tr_noop("连接"), tr_noop("在线"), Colors.GOOD)

  def _update_panda_status(self):
    if ui_state.panda_type == log.PandaState.PandaType.unknown:
      self._panda_status.update(tr_noop("车辆"), tr_noop("未识别"), Colors.DANGER)
    else:
      self._panda_status.update(tr_noop("车辆"), tr_noop("在线"), Colors.GOOD)

...
- **实时画面**：「投屏」= 低延迟 MJPEG 流，把设备屏幕显示的内容原样镜像到手机/电脑（不连摄像头，延迟 ~100ms），**画面以 2160×1080 横屏宽屏显示**（无竖横切换）。投屏源三路自动兜底、跨源选最新帧（画面实时刷新）：**cereal `uiDebug.frame`**（openpilot 原生屏幕帧广播）→ **`/dev/shm` 共享内存扫描**（uiDebug 固定时自动回退到此活源）→ **ffmpeg(fbdev/kmsgrab)** 回退；即使 ffmpeg 不支持 kmsgrab、无 `/dev/fb0` 也能投屏。状态栏会显示「最后更新 Xs前」帮你判断源是否实时。「数据 HUD」= 纯数据仪表盘（车速/车道/前车/TTC）。缺 ffmpeg 时点「一键安装」即可，不用手动 ssh。

...
def calibration_incomplete_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  first_word = '重新校准' if sm['extrinsicsCalibration'].calStatus == log.ExtrinsicsCalibration.Status.recalibrating else '自动校准'
  return Alert(
...
def personality_changed_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  personality_display = {
    ...
    "relaxed": "放松",
    "moreRelaxed": "舒适",
  }.get(str(personality), str(personality).title())
  return NormalPermanentAlert(f"驾驶风格: {personality_display}", duration=1.5)
...
EVENTS: dict[int, dict[str, Alert | AlertCallbackType]] = {
  ...
  EventName.bigModelLoading: {
    ET.NO_ENTRY: NoEntryAlert("大模型加载中"),
  },

  EventName.bigModelFailed: {
    ET.SOFT_DISABLE: soft_disable_alert("大模型加载失败"),
    ET.PERMANENT: NormalPermanentAlert("大模型加载失败 ", "请尝试重启车辆，\n当前已切换至普通模型驾驶", duration=20.),
  },
  ...
  EventName.fanMalfunction: {
    ET.PERMANENT: NormalPermanentAlert("散热风扇故障", "请联系 comma.ai/support"),
  },
  ...
  EventName.carNotReady: {
    ET.NO_ENTRY: NoEntryAlert("车辆未就绪"),
  },
  ...
  EventName.relayMalfunction: {
    ...
    ET.PERMANENT: NormalPermanentAlert("Harness Relay Malfunction", "请联系 comma.ai/support"),

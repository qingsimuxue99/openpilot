...
- **实时画面**：「投屏」= 低延迟 MJPEG 流，把设备屏幕显示的内容原样镜像到手机/电脑（不连摄像头，延迟 ~100ms），支持「横屏/竖屏」一键切换；支持 fbdev 与 DRM（kmsgrab）双路自动探测，无 `/dev/fb0` 的设备也能尝试抓取。「数据 HUD」= 纯数据仪表盘（车速/车道/前车/TTC）。缺 ffmpeg 时点「一键安装」即可，不用手动 ssh。
